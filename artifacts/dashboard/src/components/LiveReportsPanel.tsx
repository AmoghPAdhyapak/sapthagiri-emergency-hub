...
import {
  AlertTriangle, Clock, Stethoscope, Loader2, RefreshCw,
  ShieldAlert, CheckCircle2, Eye, Skull, IdCard, History,
...
const CACHE_KEY = "HOSPITAL_DB_ENCOUNTERS_RED";
...
export function LiveReportsPanel() {
  ...
  const fetchEncounters = () => {
    ...
    fetch("/api/triage/encounters")
      ...
      .then((data: EncounterRecord[]) => {
        const red = data.filter((e) => e.triageLevel === "RED" && !e.isArchived);
        if (red.length > 0) {
          setEncounters(red);
          try { localStorage.setItem(CACHE_KEY, JSON.stringify(red)); } catch { /* ignore */ }
        } else {
          try {
            const cached = localStorage.getItem(CACHE_KEY);
            if (cached) {
              const restored = JSON.parse(cached) as EncounterRecord[];
  ...
  const handleComplete = async (encounterId: string) => {
    ...
    try {
      ...
      setExpanded((prev) => { const n = new Set(prev); n.delete(encounterId); return n; });
    } catch {
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Critical patients requiring immediate attention. Click any row to view the full operational file.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-red-400">{encounters.length} CRITICAL</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-sm text-muted-foreground/60 mt-1">Critical triage encounters appear here as they come in.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {encounters.map((enc) => {
            ...
            return (
              <Card
                key={enc.encounterId}
                className={`border transition-all ${isExpanded ? "border-red-500/80 shadow-[0_0_24px_hsl(0_80%_50%_/_0.2)]" : "border-red-500/60 bg-red-500/8 shadow-[0_0_20px_hsl(0_80%_50%_/_0.15)]"}`}
              >
                <CardContent className="p-0">
                  {/* ── Clickable header row ── */}
                  <div
                    onClick={() => toggleExpand(enc.encounterId)}
                    className="p-4 cursor-pointer hover:bg-red-500/5 transition-colors select-none"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0 bg-red-500 animate-ping" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-0.5">
                            ...
                            <Badge variant="outline" className="text-xs font-bold bg-red-500/20 text-red-300 border-red-500/40">RED</Badge>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {allergies.length > 0 && (
                          <span className="text-[10px] font-bold text-red-400 bg-red-500/10 border border-red-500/30 rounded px-1.5 py-0.5">
                            ⚠ ALLERGY
                          </span>
                        )}
                  ...
                  {isExpanded && (
                    <div className="border-t border-red-500/30 bg-slate-950/60 space-y-4 p-4">
                      {/* Symptoms + Verdict */}
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-800 pb-4">
                        ...
                        <div>
                          ...
                          <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-red-950 text-red-400 border border-red-900">
                            {enc.symptomFinalVerdict ?? "RED — Critical Emergency"}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="border-b border-slate-800 pb-4">
                          ...
                          {enc.crossHospitalContinuityLogs.slice(0, 3).map((c) => (
                            <div key={c.id} className="text-xs text-slate-400 bg-slate-900/50 rounded px-2 py-1 mb-1">
                              <span className="font-semibold text-red-400">{c.doctorName}</span>: {c.notes.slice(0, 100)}{c.notes.length > 100 ? "…" : ""}
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        ...
                        {!state ? (
                          <Button
                            size="sm"
                            variant="outline"
                            className="w-full border-red-500/40 text-red-300 hover:bg-red-500/20 text-xs"
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-1.5">
                              <Label className="text-xs uppercase tracking-wider text-muted-foreground font-bold">Clinical Outcome</Label>
                                  ...
                                  <button
                                    ...
                                    className={`flex flex-col items-center gap-1.5 p-2.5 rounded-lg border text-xs font-semibold transition-all ${
                                      state.selectedAction === action
                                        ? `${color} ring-2 ring-offset-1 ring-offset-background ${action === "OUT_OF_DANGER" ? "ring-emerald-500" : action === "UNDER_OBSERVATION" ? "ring-yellow-500" : "ring-red-700"}`
                            ...
                            <div className="flex gap-2">
                              <Button className="flex-1" size="sm" disabled={state.loading} onClick={() => handleComplete(enc.encounterId)}>
                                {state.loading
