...
import {
  Eye, Clock, Stethoscope, Loader2, RefreshCw,
  CheckCircle2, ArrowUpCircle, IdCard, AlertTriangle,
...
const CACHE_KEY = "HOSPITAL_DB_ENCOUNTERS_YELLOW";
...
const ACTION_OPTIONS: { action: QueueAction; label: string; color: string; icon: React.ReactNode; desc: string }[] = [
  ...
  },
  {
    action: "ESCALATE",
    label: "Escalate → RED",
    desc: "Condition worsened — move to Emergency Hub",
...
export function ObservationQueuePanel() {
  ...
  const fetchEncounters = () => {
    ...
    fetch("/api/triage/encounters")
      ...
      .then((data: EncounterRecord[]) => {
        const yellow = data.filter((e) => e.triageLevel === "YELLOW" && !e.isArchived);
        if (yellow.length > 0) {
          setEncounters(yellow);
          try { localStorage.setItem(CACHE_KEY, JSON.stringify(yellow)); } catch { /* ignore */ }
        } else {
          try {
  ...
  const handleConfirm = async (encounterId: string) => {
    ...
    if (!state.reason.trim()) { updateAction(encounterId, { error: "Clinical reasoning is mandatory — document the reason for this status change." }); return; }
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Urgent patients under active monitoring. Click any row to view the full clinical file.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-yellow-400">{encounters.length} URGENT</span>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-muted-foreground">
          <Loader2 className="w-6 h-6 animate-spin mr-3" /> Loading observation queue...
        </div>
      ) : encounters.length === 0 ? (
        <div className="text-center py-20">
          <CheckCircle2 className="w-12 h-12 text-yellow-400/30 mx-auto mb-3" />
          <p className="text-muted-foreground font-medium">No YELLOW patients in the observation queue.</p>
          <p className="text-sm text-muted-foreground/60 mt-1">YELLOW triage encounters appear here as they arrive.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {encounters.map((enc) => {
            ...
            return (
              <Card key={enc.encounterId} className={`border transition-all ${isExpanded ? "border-yellow-500/70" : "border-yellow-500/40 bg-yellow-500/5"}`}>
                <CardContent className="p-0">
                  {/* ── Clickable header row ── */}
                  <div
                    onClick={() => toggleExpand(enc.encounterId)}
                    className="p-4 cursor-pointer hover:bg-yellow-500/5 transition-colors select-none"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0 bg-yellow-500" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-0.5">
                            ...
                            <Badge variant="outline" className="text-xs font-bold bg-yellow-500/20 text-yellow-300 border-yellow-500/40">YELLOW</Badge>
                            ...
                            <span className="text-xs font-black uppercase tracking-widest text-yellow-400 flex items-center gap-1">
                  ...
                  {isExpanded && (
                    <div className="border-t border-yellow-500/20 bg-slate-950/60 space-y-4 p-4">
                      {/* Symptoms + Verdict */}
                      <div className="grid grid-cols-2 gap-4 border-b border-slate-800 pb-4">
                        ...
                        <div>
                          ...
                          <span className="inline-block px-2 py-0.5 rounded text-[11px] font-bold bg-yellow-950 text-yellow-400 border border-yellow-900">
                            {enc.symptomFinalVerdict ?? "YELLOW — Urgent Care Required"}
                          </span>
                        </div>
                      </div>
                            ...
                            {enc.statusHistory.map((h) => (
                              <div key={h.eventId} className="text-[11px]">
                                <div className="flex items-start justify-between gap-2">
                                  <span className="text-slate-400">▪ <span className="text-yellow-400">{h.action}</span>{h.doctorId ? ` — Dr. ${h.doctorId}` : ""}</span>
                        ...
                        <div className="border-b border-slate-800 pb-4">
                          ...
                          {enc.crossHospitalContinuityLogs.slice(0, 3).map((c) => (
                            <div key={c.id} className="text-xs text-slate-400 bg-slate-900/50 rounded px-2 py-1 mb-1">
                              <span className="font-semibold text-yellow-400">{c.doctorName}</span>: {c.notes.slice(0, 100)}{c.notes.length > 100 ? "…" : ""}
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        ...
                        {!state ? (
                          <Button
                            size="sm"
                            variant="outline"
                            className="w-full border-yellow-500/40 text-yellow-300 hover:bg-yellow-500/20 text-xs"
                                  ...
                                  <button
                                    ...
                                    className={`flex flex-col items-center gap-1.5 p-2.5 rounded-lg border text-xs font-semibold transition-all ${
                                      state.selectedAction === action
                                        ? `${color} ring-2 ring-offset-1 ring-offset-background ${action === "COMPLETED" ? "ring-emerald-500" : action === "ESCALATE" ? "ring-red-500" : "ring-yellow-500"}`
