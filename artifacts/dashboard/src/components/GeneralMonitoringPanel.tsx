  ...
  const fetchEncounters = () => {
    ...
    fetch("/api/triage/encounters")
      ...
      .then((data: EncounterRecord[]) => {
        setEncounters(data.filter((e) => e.triageLevel === "GREEN" && !e.isArchived));
        setLoading(false);
      })
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Stable patients — routine consultation and monitoring. Doctors can complete or escalate.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {encounters.map((enc) => {
            ...
            return (
              <Card key={enc.encounterId} className="border border-emerald-500/30 bg-emerald-500/5 transition-all">
                          ...
                          <div className="mt-2 pt-2 border-t border-border/30">
                            ...
                            {enc.statusHistory.slice(-3).map((h) => (
                              <div key={h.eventId} className="text-[10px] text-muted-foreground font-mono mb-0.5">
                                {new Date(h.timestamp).toLocaleTimeString()} — <span className="text-emerald-400">{h.action}</span> by {h.doctorId}
                              </div>
                            ))}
                          </div>
                        )}
